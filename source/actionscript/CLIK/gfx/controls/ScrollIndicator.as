class gfx.controls.ScrollIndicator extends gfx.core.UIComponent
{
   ...
   function configUI()
   {
      ...
      this.initSize();
      this.direction = this._rotation != 0 ? "horizontal" : "vertical";
      var _loc3_;
   ...
   function updateThumb()
   {
      ...
      var _loc6_ = Math.max(1,this.maxPosition - this.minPosition + this.pageSize);
      var _loc2_ = (this.direction != "horizontal" ? this.__height : this.__width) + this.offsetTop + this.offsetBottom;
      this.thumb.height = Math.max(10,this.pageSize / _loc6_ * _loc2_);
      var _loc4_ = (this.position - this.minPosition) / (this.maxPosition - this.minPosition);
      var _loc3_ = - this.offsetTop;
      var _loc5_ = _loc4_ * this.availableHeight + _loc3_;
      this.thumb._y = Math.max(_loc3_,Math.min(_loc2_ - this.offsetTop,_loc5_));
      this.thumb.visible = !(isNaN(_loc4_) || this.maxPosition == 0);
   }
   function onScroller()
   {
      if(this.isDragging)
