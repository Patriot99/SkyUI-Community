class QuestsPage extends MovieClip
{
   ...
   function handleInput(details, pathToFocus)
   {
      ...
      if(!_loc4_ && pathToFocus != undefined && pathToFocus.length > 0)
      {
         _loc4_ = pathToFocus[0].handleInput(details,pathToFocus.slice(1));
      }
      return _loc4_;
   }
   function onShowMap()
   ...
   function updateShowOnMapButtonAlpha(a_entryIdx)
   {
      ...
      this._showOnMapButton._alpha = _loc3_;
   }
   function onToggleQuestActive(a_bnewActiveStatus)
   {
      ...
      if(this.isViewingMiscObjectives())
      {
         ...
         {
            if(this.ObjectiveList.entryList[_loc5_].formID == _loc3_ && this.ObjectiveList.entryList[_loc5_].instance == _loc4_)
            {
   ...
   function onQuestsDataComplete(auiSavedFormID, auiSavedInstance, abAddMiscQuest, abMiscQuestActive, abAllowShowOnMap)
   {
      ...
      var _loc9_ = false;
      var _loc10_ = 0;
      while(_loc10_ < this.TitleList.entryList.length)
      {
         if(this.TitleList.entryList[_loc10_].formID == 0)
         {
            this.TitleList.entryList[_loc10_].timeIndex = 1.7976931348623157e+308;
         }
         else
         {
            this.TitleList.entryList[_loc10_].timeIndex = _loc10_;
         }
         if(this.TitleList.entryList[_loc10_].completed)
         {
            if(_loc7_ == undefined)
            {
               _loc7_ = this.TitleList.entryList[_loc10_].timeIndex - 0.5;
            }
            _loc8_ = true;
         }
         else
      ...
      if(_loc7_ != undefined && _loc8_ && _loc9_)
      {
         this.TitleList.entryList.push({divider:true,completed:true,timeIndex:_loc7_});
      }
      ...
      while(_loc10_ < this.TitleList.entryList.length)
      {
         if(this.TitleList.entryList[_loc10_].text != undefined)
         {
            this.TitleList.entryList[_loc10_].text = this.TitleList.entryList[_loc10_].text.toUpperCase();
         }
         if(this.TitleList.entryList[_loc10_].formID == auiSavedFormID && this.TitleList.entryList[_loc10_].instance == auiSavedInstance)
         {
            _loc11_ = _loc10_;
         }
         _loc10_ += 1;
   ...
   function SetDescriptionText()
   {
      ...
      {
         this.ObjectiveList._y = this.ObjectivesHeader._y + this.ObjectivesHeader._height + _loc3_;
      }
      this.ObjectiveList.border._height = Math.max(_loc4_ - this.ObjectiveList._y,_loc5_);
