class TweenMenu extends MovieClip
{
   ...
   function handleInput(details, pathToFocus)
   {
      ...
      if(!this.bClosing && Shared.GlobalFunc.IsKeyPressed(details))
      {
         ...
         if(_loc4_ > 0)
         {
            if(_loc4_ != this.Selections_mc._currentframe - 1)
            {
               this.Selections_mc.gotoAndStop(TweenMenu.FrameToLabelMap[_loc4_]);
               gfx.io.GameDelegate.call("HighlightMenu",[_loc4_]);
            }
            else
            {
               gfx.io.GameDelegate.call("OpenHighlightedMenu",[_loc4_]);
            }
         }
